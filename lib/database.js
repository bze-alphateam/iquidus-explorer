...
module.exports = {
	...
	find_mn: function (txhash, cb) {
		Mns.findOne({txhash: txhash}, function (err, mn){
			if (err) {
				return cb(null);
			} else {
				if (mn) {
	...
	create_mn: function(params, cb) {
		var newMn = new Mns(params);
		newMn.save(function(err) {
			if (err) {
				console.log(err);
				return cb();
			} else {
